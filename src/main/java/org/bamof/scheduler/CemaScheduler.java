...
public final class CemaScheduler extends SchedulerSupport {
    @Override
    public Algorithm algorithm() {
        return Algorithm.CEMA;
    }
    ...
    public void schedule(List<Task> readyTasks, List<Vm> vms, List<VmType> vmTypes, int slot, SimulationConfig config) {
        ...
        for (Task task : readyTasks) {
            Vm vm = vms.stream()
                    .filter(candidate -> canPlace(task, candidate, slot, false))
                    .min(Comparator.comparing((Vm candidate) -> candidate.idle())
                            .thenComparingDouble(candidate -> candidate.type().costPerSlot())
                            .thenComparingDouble(Vm::queuedWorkloadMi))
                    .orElse(null);
            if (vm == null) {
